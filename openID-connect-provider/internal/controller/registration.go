...
func Registration(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&client); err != nil {
		c.JSON(http.StatusNotFound, gin.H{"code": http.StatusOK, "message": "RP注册参数不正确", "data": err.Error()})
		return
	}
	//redis存储ClientId
	if err := middleware.SetString(middleware.OIDC+":"+middleware.CLIENT+":"+client.ClientId, client.ClientSecret, 0); err != nil {
		c.JSON(http.StatusNotFound, gin.H{"code": http.StatusOK, "message": "client_id注册失败", "data": err.Error()})
	} else {
